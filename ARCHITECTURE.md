...
`tools/architecture/graph.json` is the machine-readable Wave 0 authority for
planned zones, allowed imports, server/browser classification, provider locations,
composition roots, and exceptions. During Wave 1 it must gain executable import
checks. Until the root command harness exists, `python3 tools/docs/check.py`
validates the manifest and generated architecture views.
