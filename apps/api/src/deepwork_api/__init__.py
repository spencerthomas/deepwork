...
def create_app(
    *,
    ...
    The default stays in credential-free deterministic fixture mode. Passing
    both loopback arguments explicitly enables execution through a local
    ``langgraph dev`` Agent Server; anything except an explicit HTTP loopback
    IP origin is rejected before any network use.
    """
