...
class LocalAgentServerRunner:
    ...
    async def update_plan(
        ...
        *,
        ...
        try:
            ...
            state = await self.source.get_state(thread_id)
        except Exception:
            raise TaskSourceUnavailableError from None
        ...
        return PlanUpdateRecord(task.task_id, updated.run_id, interrupt.interrupt_id, record.plan)
    ...
    async def _follow(self, task: TaskSnapshot, run: LocalRun) -> None:
        try:
            ...
            async for _ in self.source.stream(run):
                await self.repository.append_event(
                    task.task_id,
