...
// Distinguish local development's unset value from the explicit empty string
// supplied by config/runtime.ts for production. Production therefore uses
// same-origin requests through the Next.js `/api/*` proxy — the only way the
// HttpOnly session cookie reaches both REST calls and the SSE stream when the
// API is hosted on a different domain than the web app.
