...
| Performance proof | 0/4 | 3/4 | `make test-performance` executes a versioned synthetic 1,000-task API-contract profile at desktop and phone widths against the production bundle. Commit `0c2b05137846987536bf5b5759b979b448d6df8d` caps observed task rows at 50 and proves URL pagination, exact full-set search results, useful keyboard opening and provisional p75 interaction below 200 ms. The main CI job installs pinned Chromium before the gate. It is local Chromium proof, not an accepted reference-device, frame, memory, long-stream, hosted-load or manual-AT qualification. | Complete the remaining `E2E-V1-10` load surfaces and obtain reference-profile acceptance |
