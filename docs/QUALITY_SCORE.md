...
| Performance proof | 0/4 | 3/4 | `make test-performance` executes versioned synthetic 1,000-task inbox and 1,001-event task-detail profiles at desktop and phone widths against the production bundle. Commit `770f923d57cc8d9a9d5465b34f75b65103597033` caps observed inbox rows at 50 and Stream rows at 100; proves full-set search, keyboard opening, stable gap-free traversal of all 11 event pages, current plan/approval retention, and useful result/source/file/detail inspection; and keeps provisional inbox p75 below 200 ms and bulk-replay readiness below 4,000 ms. The main CI job installs pinned Chromium before the gate. This is local synthetic bulk-replay proof, not a sustained incremental-stream, accepted-device, frame, retained-memory, hosted-load or manual-AT qualification. | Add incremental-stream/backpressure and memory/frame/long-task reports, complete the remaining `E2E-V1-10` load surfaces, and obtain reference-profile acceptance |
