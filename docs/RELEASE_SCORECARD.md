...
| `E2E-V1-11-CONTRIBUTOR` | Partial | No | No | No | The supported root `make check` is green: architecture enforcement, formatting, lint, type checks, 80 domain tests, 73 SDK tests, 32 UI tests, 329 web tests, 371 passing API tests plus the 101-passing contract-only gate, 100 agent tests and reproducible package builds all pass under the required Node 24/pnpm 11 toolchain. Root `make doctor` now fails closed on unsupported Node/pnpm versions, passes under the supported toolchain and has version-contract regressions in the root gate. Two independent clean-machine contributor runs, intentional drift repair and license/trademark proof remain open. |
