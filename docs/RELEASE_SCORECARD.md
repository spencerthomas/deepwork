...
| Hosted golden journey | `make test-hosted` | Protected run unavailable: no reviewed hosted URL/access key is configured, and the deployed API/web services have not published the expected build SHA. The gate is not run and the hosted column remains `No`. | Exact API and web build identity equal to `DEEPWORK_EXPECTED_BUILD_SHA`; authenticated non-fixture runtime and required source capabilities; real registry choice retained through detail/reopen; non-empty evidence; complete result/files/trace/reopen; zero browser page/console errors; zero failed API requests or non-success API responses. It proves these only when the protected environment supplies URL/key and both deployed services report the checked-out commit. |
