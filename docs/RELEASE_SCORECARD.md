...
their two-identity browser restart proof is pinned at `b803fcb`.
...
| `E2E-V1-11-CONTRIBUTOR` | Partial | No | No | No | The supported root `make check` is green: architecture enforcement, formatting, lint, type checks, 80 domain tests, 66 SDK tests, 32 UI tests, 324 web tests, 362 passing API tests plus the 92-passing contract-only gate, 100 agent tests and reproducible package builds all pass under the required Node 24/pnpm 11 toolchain. The root `make doctor` still needs to fail on unsupported Node/pnpm versions, and two independent clean-machine contributor runs, intentional drift repair and license/trademark proof remain open. |
