...
`770f923d57cc8d9a9d5465b34f75b65103597033` (feature commit
`361ccaf6cb0000b3953075b42650ddde3c40743b`), reviewed 2026-08-03 PDT.
...
| `E2E-V1-10-PERFORMANCE` | Partial | Partial | No | No | `make test-performance` runs a versioned synthetic 1,000-task inbox and 1,001-event task-detail profile against the production web bundle at 1440x1000 and 390x844. URL-restorable inbox paging caps the DOM at 50 rows while full-set search and exact-result keyboard opening remain usable; the latest local search p75 was 22.80 ms desktop and 18.39 ms phone against the provisional 200 ms budget. The task thread keeps at most 100 recent/active-review items, Stream keeps at most 100 rows, stable cursor-anchored history does not shift under live appends, and the browser gate proves all 1,001 ordered event IDs across 11 pages from latest to oldest and back. Useful result, Sources, Files and Details remain inspectable; local bulk-replay readiness was 232.62 ms desktop and 232.00 ms phone against the provisional 4,000 ms budget. The gate is CI-blocking. This remains synthetic local bulk-replay proof, not a sustained incremental-stream, retained-memory/frame/long-task, multi-source, large subagent/file/diff, reconnect, reference-device, hosted-load or manual-AT qualification. |
| `E2E-V1-11-CONTRIBUTOR` | Partial | No | No | No | The supported root `make check` is green: architecture enforcement, formatting, lint, type checks, 74 domain tests, 60 SDK tests, 32 UI tests, 304 web tests, 314 API tests plus 77 API contract tests, 100 agent tests and reproducible package builds all pass. Two independent clean-machine contributor runs, intentional drift repair and license/trademark proof remain open. |
...
| Inbox and long-stream performance | `make test-performance` | Passing locally at desktop and phone widths and required by the main CI verification job | Versioned synthetic 1,000-task and 1,001-event profiles; maximum 50 inbox rows and 100 Stream rows; stable complete history traversal; useful result/source/file/detail inspection; provisional interaction and bulk-replay budgets. Not sustained incremental-stream, accepted-device, frame, retained-memory, hosted-load or manual-AT proof |
