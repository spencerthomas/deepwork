...
| Route and journey visuals | `make test-visual` | The earlier six-case production-browser gate passed. On `1a80096`, the contract-only gate passes 2 tests across all 12 desktop/phone routes and proves color/layout mutations fail; the production-browser rerun is blocked by the workstation `ENOSPC` condition and is not claimed current-head green. Required by the separate `visual-acceptance` CI job | Immutable prototype source, complete route mapping, full-resolution sRGB comparison over the common viewport, reviewed route thresholds, desktop/phone screenshot contract including the four-agent source-registry Fleet, uncertain-dispatch recovery, 320px reflow and golden-journey states |
