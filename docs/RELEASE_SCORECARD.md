...
`32b4e2a5dc7eac7c3a01ab14f5f1f99056404aca`. The application now creates its
...
request-cancellation and silent-settled-stream cases are covered. This is local
sequential proof. Multi-process ownership leases, bounded receipt retention,
transient-outage retry, OS-process kill, real-provider and hosted proof remain
open, so no scenario changes to `Yes`.
