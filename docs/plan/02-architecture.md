...
- **Reliability stack** (the fault-tolerance middleware set, adopted wholesale): `ModelRetryMiddleware` + `ToolRetryMiddleware` (transient errors, exponential backoff), `ToolErrorMiddleware` (LLM-recoverable failures fed back as error ToolMessages), `ModelFallbackMiddleware` (provider outage), `ModelCallLimitMiddleware`/`ToolCallLimitMiddleware` (runaway caps, surfaced in the UI). Directly addresses open-swe v1's top failure class.
- **Verification**: `RubricMiddleware` (harness beta, ≥0.6.5) — LLM-as-judge grading loop against a per-task rubric until satisfied or capped; templates ship rubric defaults, the composer accepts custom ones, and the task detail shows verdicts/iterations. `CodeInterpreterMiddleware` (QuickJS, beta) enabled per template for research/data work: programmatic tool calling and dynamic subagent fan-out from code, keeping intermediate results out of context.
- **Parallel workstreams (v1.x)**: async subagents (preview) — supervisor launches background tasks over Agent Protocol (`start/check/update/cancel/list_async_tasks`), tracked in a compaction-proof `async_tasks` state channel; co-deployed graphs via ASGI transport. v1 gets parallelism via separate tasks; the supervisor pattern lands behind a flag. See the [feature map](08-deepagents-feature-map.md).
...
- **Goals & rubrics adopted into the task model**: dcode's `/goal` (agent drafts acceptance criteria → human review → per-turn grading until done) and `/rubric` become Deep Work's richer plan-approval: tasks carry a goal or rubric, graded server-side, surfaced as the verification panel. The engine is the harness's own `RubricMiddleware` (beta, ≥0.6.5) — a direct include in `packages/agent`, not new machinery; dcode's goal *lifecycle* (draft → review → amend/pause) is the UX reference layered on top.
