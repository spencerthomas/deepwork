...
Title = first human message, truncated (the cross-UI convention, [04 L14](../../research/04-langchain-uis.md)). Preview = last narration text, else the `in_progress` todo `content`, else last tool name ([03 §3.1](../03-ui-spec.md)). Both rendered as **plain text only** (§6). Whole row is a link to `/tasks/[id]`; keyboard `j/k` + `Enter` per UI spec §7.
...
**Unread — definition & storage (D-003: no server DB; PROPOSAL, flagged provisional).** A row is unread iff `thread.updated_at > seenAt[threadId]`. Mark-seen on opening task detail; threads created from this device start seen; "Mark all read" action in the list header. Store: `localStorage["deepwork.inbox.seen.v1"]` = `{ [threadId]: ISO-8601 }`, LRU-capped at 5,000 entries; unavailable/corrupt storage degrades to all-read without error. This is **per-device only** — cross-device unread requires a per-user server-side niche (LangGraph Store user namespace / MDA per-user memory) and is deferred → §9. We explicitly do **not** write read-state into thread metadata: it is shared across viewers (team persona, [01](../01-vision.md)) and would pollute org data.
