...
- **Egress policy**: platform default is HTTP/S open + all raw TCP blocked; per-environment allow-list narrows or widens (`allow_list` opens host:port) — surfaced in the editor as levels mirroring Claude Code's None/Trusted/Full/Custom ([research 06](../../research/06-execution-sandboxes.md) fact 10; [research 03](../../research/03-competitor-teardown.md) fact 9).
...
| `egress` | `{level: "trusted"\|"custom"\|"full", allow: ["host[:port]"]}` | compiles to proxy `allow_list`/access rules; `trusted` = platform default (HTTP/S via proxy, raw TCP blocked). A "none" level is desired but unverified (§9-Q7) |
...
- **Egress is the exfiltration boundary**: default HTTP/S-only via proxy, raw TCP blocked; allow-lists narrow further; rules render in the editor, never free-form in the sandbox ([research 06](../../research/06-execution-sandboxes.md) fact 10).
