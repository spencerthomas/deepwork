...
5. **Connector access to the per-thread sandbox** — how `http(ctx)` handlers resolve the MDA-generated sandbox name (research 20 notes no public runtime API for discovery); shapes Task 2's implementation (the expired-sandbox error itself is fixed: `410` `sandbox_expired` per F11's canonical contract, §4.2).
