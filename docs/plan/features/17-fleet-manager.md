...
| needs ← | [F15 · Task templates](./15-task-templates.md) | Template catalog entries — binding ids `coding` · `research` · `writing` (+ `data-analyst`, reserved v2) — with per-template assistant-config payloads (D-014: templates are assistant configs, not codebases); gallery display names map onto these ids per §3.3 |
...
- **Key custody**: org/workspace service keys server-side only in `apps/server` (P-005, [F28](./28-backend-glue-service.md)); per-user PATs stored via the server proxy by default, client-side only in local mode with the on-screen trust story ([03 §3.6](../03-ui-spec.md)). No key ever reaches the client bundle.
...
9. Every non-API-creatable action (Fleet CRUD, gated MDA features) renders a smith.langchain.com deep link with an explanation; beta features carry visible badges (degradation-matrix audit, §3.5).
...
|---|---|---|---|
| 1 | `packages/sdk`: `AgentSummary`/`AgentConfigModel` types + capability computation + per-source aggregation with isolation, caching, backoff | [F04](./04-sdk-and-agent-sources.md) | Unit tests cover all four tiers + failure isolation; AC-1 |
...
| 4 | Hub file service in `apps/server` (P-005, [F28](./28-backend-glue-service.md)): directory get/commit proxy, version listing, conflict detection (read-before-write) | F28 skeleton | AC-4; conflict path unit-tested |
...
5. **Fleet enumeration & richer management** — is there any supported way to list a workspace's Fleet agents (the unpublished "Fleet API" behind smith.langchain.com, [research 12](../../research/12-lifecycle-auth-followup.md))? v1 ships connect-by-URL. Related unlocks: O-004 (self-config-via-chat over the public run API, probe post-M3) and whether Fleet publish webhooks (config + base64 ZIP, [research 10](../../research/10-openswe-fleet.md)) could drive auto-import.
...
7. **`interrupt_on` in assistant config** — confirm with [F14](./14-agent-package.md) that the Deep Work agent can expose `interrupt_on` through its config schema so the matrix is per-assistant (template-level) rather than requiring a revision; and how `/v1/deepagents` config-first agents overlap with `/v2/deployments` MDA rows in the index (dedup key — the two CLIs are separate layers, [research 20](../../research/20-gapfill-mda-api.md)).
8. **Post-import editing home** — after a Fleet-export ZIP is imported and deployed via tarball, `config.json`/`tools.json`/`subagents/` have no Context Hub home (the Hub sync covers instructions/skills/memories only, [research 01](../../research/01-managed-deep-agents.md)) — do subsequent edits require tarball re-upload every time, or should imports be steered to a github-source deployment for a durable editing surface?
...
| Fleet CRUD stays closed past v1 ([research 12](../../research/12-lifecycle-auth-followup.md)) | "Fleet-like manager" perception gap | Export format as the bridge; link-outs; O-004 probe; LangChain relationship as the unlock ([02 §12](../02-architecture.md)) |
