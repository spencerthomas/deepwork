...
Out of scope: Layer 0/1 foundations — `org-memory/` template, tracing conventions, org-analyst digests, Insights provisioning ([F22](./22-org-intelligence-v1.md)); general approvals-inbox mechanics ([F10](./10-approvals-inbox.md)); the `apps/server` skeleton, hosting, and shared middleware ([F28](./28-backend-glue-service.md)); push fan-out delivery (F19, see [catalog](./README.md)); Layers 3–5 (OKF/openwiki, data plane, Graphiti — v2/v3 per [07 §5](../07-org-intelligence.md)).
...
| shares problem with | F19 (push subscriptions, see [catalog](./README.md)) | Both need durable server-side state under D-003; §3.3 resolves F23's copy by making the org's Context Hub the source of truth — align the answers |
...
**Decisions**: `approve` → `apps/server` applies the diff as a commit to `org-memory/**` via the Hub API (write credential: reviewer's OAuth token if Hub write scopes allow, else server-held service key — audit-trail difference tracked in §9-Q4) and closes the proposal (removes it from the proposal area in the same commit). `edit` → reviewer amends the diff, then approve-with-edits merges the amended content. `reject` (message optional but encouraged) → no memory change; the rejection + message are recorded in the proposal log; the next consolidation run receives recent rejections in its prompt context so it stops re-proposing rejected content — the feedback loop of [07 §2](../07-org-intelligence.md). `respond` does not exist here (no live run to talk to).
...
- **Dedupe**: before proposing, the agent greps existing `org-memory/` and the open + recently rejected proposal log (both are files it can read); the manifest carries `relates_to`/`supersedes` references. Verbatim re-proposal of open or rejected content is a defect (AC-5).
...
- **Runaway volume**: circuit breaker — if open proposals exceed a threshold, the run records that fact and proposes nothing; the schedules UI shows the skip and the cron's disable switch is one click ([03 §3.5](../03-ui-spec.md)).
