...
- **Authn is the deployer's**: the cookbook minimum ships no identity layer and MDA identity presets don't apply ([02 §5](../02-architecture.md) planes 2–3 are LangSmith machinery). Default posture: compose binds the protocol server to localhost; exposure beyond that goes behind the deployer's reverse proxy. **`adapterConfig.defaultHeaders` is restricted to non-secret values**: the registry (env seed `NEXT_PUBLIC_AGENT_SOURCES` or the Settings store) is browser-visible — `NEXT_PUBLIC_*` is baked into the served bundle — so a static secret header there is public by definition. Authenticated exposure uses a session-bound mechanism the browser carries implicitly (same-origin session cookie at the reverse proxy, mTLS, network allowlist) or a server-side proxy that holds the credential (`apps/server` passthrough, [F28](./28-backend-glue-service.md), when the optional profile runs). Single-operator assumption documented; multi-user is out of scope (§9-Q7).
...
8. Compose defaults are localhost-bound; the reverse-proxy recipe is documented and tested with a session-bound auth mechanism (cookie or mTLS); a bundle-grep audit asserts no credential appears in `NEXT_PUBLIC_AGENT_SOURCES` or any `defaultHeaders`.
