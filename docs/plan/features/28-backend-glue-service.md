...
- **Timeouts**: short connect timeout (proposed 10 s); no per-chunk read timeout that would kill slow-but-alive streams, but the relay enforces a server-side **idle timeout** (proposed 5 min with zero upstream bytes — half-open-upstream protection, so a dead peer can't pin a relay forever) and a **max stream lifetime** (proposed 2 h); both terminate the SSE cleanly and the client rejoins via `Last-Event-ID`; idle-*reconnect* UX stays client-owned (`streamIdleReconnect`, [03 §5](../03-ui-spec.md)). Client disconnect cancels the upstream request (httpx client closed on task cancellation) so abandoned streams don't leak upstream connections. Idle/lifetime reaping is asserted by the soak + disconnect tests (§7-6).
...
6. Soak: N concurrent relayed streams (N per the §3.2 scale target, pinned in the test) for 30 min with one deliberately slow client and one half-open upstream — flat memory, no upstream connection leak; the idle timeout reaps the half-open relay and the max-lifetime cutoff terminates cleanly with a client rejoin.
