...
|---|---|---|
| The prototype sign-in screen offers OAuth, API key, workspace selection, and a desktop-device-flow message, but all paths only navigate to `/tasks`. | Prototype evidence at `26c698b`; simulated | Preserve the interaction intent, not its implied contracts. |
