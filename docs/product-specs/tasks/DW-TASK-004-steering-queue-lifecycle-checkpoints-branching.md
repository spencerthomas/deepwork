...
|---|---|---|
| The prototype keeps a composer visible during a fixture run, but it does not implement provider queue, interrupt, cancel, retry, archive, or checkpoint behavior. | Prototype evidence at `26c698b`; simulated | Retain the always-available steering concept, not the implied action semantics. |
