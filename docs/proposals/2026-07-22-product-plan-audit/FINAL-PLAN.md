...
status: proposed-final-for-review
...
Approve this document as the program-level plan for integrating the staged Deep
Work planning library. Approval does **not** authorize feature implementation or
silently make an external contract valid. It authorizes a deliberate integration
change that:

1. replaces contradictory canonical planning with the accepted decisions here;
2. installs a harness-oriented repository knowledge system;
3. preserves the detailed feature plans as the source of feature-level scope and
   acceptance;
4. turns the current 19-unit delivery plan into a program index rather than a
   second implementation authority; and
5. pilots Symphony as the development orchestrator only after its tracker,
   workspace, permissions, and recovery contract passes a bounded spike.

Until that review occurs, this document and every linked artifact remain isolated
inside this proposal directory. No runtime code, canonical plan, root instruction,
or sibling prototype is changed by this package.
...
The proposal is pinned to the repositories and hashes in the
[source ledger](catalog/source-ledger.md). Product and contract decisions use this
precedence:
...
[decision register](catalog/decision-register.md). The decisions that shape the
whole program are:
...
The full proposal is [application-architecture.md](architecture/application-architecture.md).
...
this proposal are staged drafts or sources; the tree is not claimed to exist yet.

```text
deepwork/
├── AGENTS.md                         # short map and invariant summary
├── ARCHITECTURE.md                   # canonical system/package/layer graph
├── WORKFLOW.md                       # Symphony policy and prompt, after spike
├── CONTRIBUTING.md
├── CODE_OF_CONDUCT.md
...
The [feature catalog](catalog/feature-catalog.md) is the one-owner index and the
[coverage matrix](catalog/coverage-matrix.md) traces every current route, navigation
...
### Wave 0 — Plan lock and Harness installation
...
1. Restore G0 evidence integrity and accept/amend this proposal's decisions.
2. Reconcile legacy vision, architecture, UI, roadmap, OSS, frontend, research,
   and delivery plans into the new knowledge taxonomy.
3. Install short root/nested `AGENTS.md`, root `ARCHITECTURE.md`, topical docs,
   product/design/reference/generated indexes, ExecPlan template, quality score,
   and debt tracker.
4. Approve architecture/docs rule manifests, work-item/scenario IDs, governed-path
   freshness, proof packet, and manual-dispatch policy.
5. Run the Symphony spike. Install `WORKFLOW.md` only when the tracker/security/
   workspace/recovery policy is accepted.
...
[acceptance-scenario index](catalog/acceptance-scenario-index.md) gives each
...
[merge map](integration/merge-map.md). The intended order is:
...
## 15. Open gates before implementation readiness

The program is deliberately `not-ready` until reviewers resolve:

- acceptance/amendment of the Python application service, Postgres/outbox/object,
  package graph, PWA, and Tauri decisions;
- OSS license, governance, support, version, and compatibility policy;
- exact supported Node/Python/Rust/package versions at scaffold time;
- owners and environments for each contract spike in the decision register;
- data retention/deletion, tenancy, backup/restore, notification, and artifact
  policies;
- minimum supported browser/OS/device and quantitative performance budgets;
- Symphony tracker adapter, issue schema, credential isolation, workspace safety,
  concurrency, reconciliation/retry, and human-review policy; and
- current workspace integrity: the protected untracked delivery-plan artifact and
  unrelated `docs/plans/` files changed concurrently during proposal work. This
  proposal did not edit or remove those files, but global isolation gate G0 cannot
  pass until their owner reconciles the baseline.

## 16. Review checklist

- [ ] Product accepts the v1 outcome, audience, IA, lifecycle actions, settings
      dispositions, and future boundary.
- [ ] Architecture accepts backend/frontend/desktop/mobile, package edges, state,
      credential, stream, worker, object, and deployment boundaries.
- [ ] Runtime-contract owner accepts classic baseline, gated adapters, every
      unsupported-assumption removal, spike, and fallback.
- [ ] Security accepts tenant/auth/secret/object/sandbox/GitHub/webhook/update and
      orchestration credential boundaries.
- [ ] Frontend/accessibility accepts responsive states, PWA behavior, design
      system, performance, and migration method.
- [ ] OSS maintainers accept contributor method, docs harness, commands, package
      compatibility, issue/PR process, Symphony pilot, and community policy.
- [ ] Each v1 criterion maps to one executable scenario and one retained proof
      location.
- [ ] Catalog and coverage validation report zero duplicate owners, missing
      surfaces, missing feature IDs, and broken links.
- [ ] G0 workspace integrity is restored before integration.

## 17. Detailed sources

- [Frontend audit](audits/01-frontend.md)
- [LangChain contract audit](audits/02-langchain-contracts.md)
- [Vision and scope audit](audits/03-vision-and-scope.md)
- [Existing-plan audit](audits/04-existing-plans.md)
- [Reference-code practice audit](audits/05-reference-code-practices.md)
- [Harness and orchestration audit](audits/06-harness-and-orchestration.md)
- [Application architecture](architecture/application-architecture.md)
- [Engineering and contributor conventions](architecture/engineering-conventions.md)
- [Concise root architecture draft](harness/proposed-ARCHITECTURE.md)
- [Living ExecPlan template](harness/exec-plan-template.md)
- [Fail-closed Symphony workflow draft](harness/proposed-WORKFLOW.md)
- [Symphony work-item template](harness/symphony-work-item-template.md)
- [Source ledger](catalog/source-ledger.md)
- [Decision and spike register](catalog/decision-register.md)
- [Feature catalog](catalog/feature-catalog.md)
- [Coverage matrix](catalog/coverage-matrix.md)
- [Acceptance scenario index](catalog/acceptance-scenario-index.md)
- [Staged agent guidance](agent-guidance/placement-map.md)
- [Post-review merge map](integration/merge-map.md)
- [Current proposal integrity report](integration/integrity-report.md)
