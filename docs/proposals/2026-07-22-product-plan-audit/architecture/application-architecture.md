...
evidence_pins:
  frontend: 8866d39a2888e358091208063693f260cff6d261
...
> **Review status:** this is a non-canonical staged proposal. It does not authorize runtime work, change the existing vision or roadmap, or select a hosting vendor. The package remains `not-ready` while a proposed implementation choice lacks review or an external contract lacks an accepted pinned-package/live-contract fixture.
...
## 20. Staged architecture decisions

No row is canonical until reviewed and merged through `integration/merge-map.md`.

| ADR | Decision | Status in this package | Verification before implementation |
|---|---|---|---|
| ARCH-001 | Python 3.12 FastAPI/Pydantic/SQLAlchemy/Alembic/PostgreSQL application service | Proposed for acceptance; aligns `DEC-002` | Package skeleton, async DB/migration, OpenAPI, deployment smoke |
| ARCH-002 | Separate Python Deep Agents package | Proposed for acceptance; aligns `DEC-003` | Clean wheel/install and classic deployment smoke using public APIs |
| ARCH-003 | Next.js 16/React 19 App Router for web/PWA | Proposed for acceptance; aligns `DEC-004` | Streaming vertical slice, bundle/performance/a11y qualification |
| ARCH-004 | Add pure `packages/domain` and keep SDK transport-specific | Proposed correction | Dependency-boundary lint and generated DTO mapping spike |
| ARCH-005 | Browser uses FastAPI `/api/v1`; no direct source secrets or generic proxy | Proposed security invariant | Threat model, OpenAPI secret scan, cross-tenant/SSRF tests |
| ARCH-006 | Classic deployment adapter is the v1 baseline; MDA/Fleet gated | Proposed for acceptance; aligns `DEC-001` | Named source/auth/stream/HITL spikes; MDA/Fleet independently gated |
| ARCH-007 | API and worker entry points share one distribution/image initially | Proposed implementation choice | Job/outbox load, shutdown, migration, and deploy topology spike |
| ARCH-008 | PostgreSQL outbox and leased jobs; no broker initially | Proposed implementation choice | Contention/retry/recovery test against accepted workload |
| ARCH-009 | S3-compatible object abstraction for app-owned uploads/exports | Proposed; vendor deferred | Attachment/object security and transfer spike |
| ARCH-010 | Normalized SSE task stream plus HTTP mutations | Proposed application contract; source recovery gated | Protocol-v2/legacy/join golden transcripts and proxy timeout tests |
| ARCH-011 | Responsive web is complete v1 mobile; install/PWA/push are qualified enhancements | Proposed for acceptance; aligns `DEC-006` | Browser/device, offline, push, a11y, and journey qualification |
| ARCH-012 | Tauri v2 exact remote-origin thin host | Contract-gated proposal; aligns `DEC-005` | `SPIKE-DESKTOP-001` on every supported OS; hold desktop on failure |
...
| ARCH-014 | TanStack Query for ordinary server state; dedicated stream reducer | Proposed implementation choice | SSR/hydration/cache partition/cancel/bundle proof |
| ARCH-015 | Team-compatible tenant schema with narrow initial roles | Proposed for acceptance | Authorization matrix and cross-tenant negative suite |
...
### A0 — Architecture acceptance and contract harness
...
This document can be proposed for integration only when reviewers can answer yes to all of the following:
...
Until those conditions and required spikes are accepted, the architecture remains a staged proposal and no dependent feature plan should be marked implementation-ready.
