...
Status: **staged proposal; non-canonical**
Created: 2026-07-23
Applies after approval: proposed Deep Work monorepo, application service, agent package, web/PWA, desktop host, shared TypeScript packages, tests, documentation, and contribution workflow
Runtime or canonical files changed by this document: **none**

This document proposes how Deep Work should feel to engineers who already contribute to LangChain, LangGraph, Deep Agents, or LangChain.js. It deliberately adopts the community's working method—typed public boundaries, package-local tooling, standard adapter tests, executable examples, disciplined pull requests, and reproducible releases—without inheriting compatibility obligations that a new application does not yet have.

The proposal is not a claim that these tools or commands already exist. Every command below is a contract for the future scaffold. The proposal becomes enforceable only after review, an accepted merge-map change, and corresponding configuration in the canonical repository.
...
## 18. Review decisions required before integration

This proposal recommends defaults but remains non-canonical until reviewers explicitly accept:
...
Until those decisions are accepted and enforced, this document is review material only. It must not be cited as current repository policy, and no existing canonical plan should be silently rewritten to match it.
