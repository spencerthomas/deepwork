...
The 13 positive cases, 55 file-indexed negatives, and 143 deterministic semantic
probes are indexed by `corpus.json`, `negative/matrix.json`, and
`negative/semantic-matrix.json`. All 198 negative IDs form one unique inventory,
...
record-ID uniqueness, exact available-capability name and state inventories,
coordinated interrupt signatures, type-exact JSON scalar
