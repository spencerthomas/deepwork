...
The 13 positive cases, 55 file-indexed negatives, and 144 deterministic semantic
probes are indexed by `corpus.json`, `negative/matrix.json`, and
`negative/semantic-matrix.json`. All 199 negative IDs form one unique inventory,
and every check requires exactly one declared stable diagnostic. The negatives
include tool correlation, bounded
...
record-ID uniqueness, exact capability names, states, and manifest path
identities, coordinated interrupt signatures, type-exact JSON scalar
