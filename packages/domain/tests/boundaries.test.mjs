...
describe("domain negative boundary fixtures", () => {
  for (const fixture of negativeFixtures) {
    it(`reports actionable rules for ${fixture.path}`, async () => {
      ...
      for (const { message } of violations) {
        ...
        expect(message).toContain("pnpm --filter @deepwork/domain check-architecture");
      }
    });
  }
  ...
  it("fails closed on malformed source with actionable help", () => {
    ...
    expect(failure.message).toContain("pnpm --filter @deepwork/domain check-architecture");
  });
});
