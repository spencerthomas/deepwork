...
import { inspectSource, negativeFixtures } from "../scripts/check-boundaries.mjs";

const packageRoot = fileURLToPath(new URL("../", import.meta.url));

describe("SDK negative boundary fixtures", () => {
  for (const fixture of negativeFixtures) {
    it(`reports actionable rules for ${fixture.path}`, async () => {
      ...
      const violations = inspectSource(source, join(packageRoot, fixture.path));
      ...
      for (const { message } of violations) {
        ...
        expect(message).toContain("pnpm --filter @deepwork/sdk check-architecture");
      }
    });
  }
  ...
  it("fails closed on malformed source with actionable help", () => {
    ...
    expect(failure.message).toContain("pnpm --filter @deepwork/sdk check-architecture");
  });
});
