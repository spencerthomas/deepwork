...
export function localProductionWebServer(readyPath: string) {
  return {
    command:
      `DEEPWORK_EPHEMERAL_ACCEPTANCE=1 NEXT_PUBLIC_API_BASE_URL= DEEPWORK_API_ORIGIN=http://127.0.0.1:8000 ${node} apps/web/node_modules/next/dist/bin/next build apps/web --webpack && ` +
