...
NAME = "deepwork_coding_review_surface_spikes"
VERSION = "0.1.0"
...
def _metadata() -> bytes:
    return (
        ...
        "Name: deepwork-coding-review-surface-spikes\n"
        f"Version: {VERSION}\n"
...
def build_wheel(wheel_directory: str, config_settings=None, metadata_directory=None) -> str:
    ...
    for path in sorted((root / "src/coding_review_surface_spikes").glob("*.py")):
        files[f"coding_review_surface_spikes/{path.name}"] = path.read_bytes()
    ...
    files[f"{DIST_INFO}/entry_points.txt"] = (
        "[console_scripts]\n"
        "pytest = coding_review_surface_spikes.pytest_runner:main\n"
    ).encode()
